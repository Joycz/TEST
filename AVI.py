        ...
        try:
            ...
            try:
                if kiemtra > 2:
                    ...
                    ##################### cần fix lỗi hiện hộp thoại spam video cần chờ time ###########################################
                    time.sleep(1.5)
                    try:
                        popup_element = driver.find_element(By.ID, 'popup')
                        # Nếu popup xuất hiện, thực hiện thao tác tương ứng
                        print("Popup xuất hiện, thực hiện thao tác...")
                        # Tìm và click vào button__title
                        button_element = popup_element.find_element(By.CLASS_NAME, 'button__title')
                        button_element.click()
                    except NoSuchElementException:
                        # Nếu không có popup xuất hiện, tiếp tục thực hiện các thao tác khác
                        print("Không có popup xuất hiện, tiếp tục thực hiện các thao tác khác...")
                    time.sleep(1)
                    driver.switch_to.window(driver.window_handles[1])
                else:
                    print("Hết nhiệm vụ!")
            ...
            try:
                ...
                continue
            '''' ---------------------- '''
            try: #Lay thoi gian va chuyen vao iframe
                time_job = int(driver.find_element(By.ID,'tmr').text)
                iframe = driver.find_element(By.ID,"video-start")
                driver.switch_to.frame(iframe)
                #An nut play YT sau khi chuyen vao iframe
                play_button_elements = driver.find_elements(By.CLASS_NAME,"ytp-large-play-button") 
                found = False #bool ở đây giống như flag (cờ)
                for element in play_button_elements: #duyệt từng phần tử có nút play YT lớn
                    classes = element.get_attribute("class") #lấy về giá trị attribute của nó
                    if "ytp-large-play-button-red-bg" in classes: #nếu element này nằm trong class
                        found = True 
                        break 
                if found: # Nếu found True
                    wait = WebDriverWait(driver, 25)
                    play_button = wait.until(EC.presence_of_element_located((By.CLASS_NAME, "ytp-large-play-button"))) #lấy element play button và chờ nó xuất hiện
                    play_button.click() #Sau khi chờ thì click vào
